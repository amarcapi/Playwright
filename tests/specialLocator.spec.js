...
test('Choose products dynamically', async ({page})=>
{
    ...
    const prices = await page.$$eval('.current-price', prices => prices.map(price => parseFloat(price.innerText.replace('$', ''))));
    const lowestPriceIndex = prices.indexOf(Math.min(...prices));
    await page.click(`.product-link .product:nth-child(${lowestPriceIndex + 1})`);
